...
export default async function requestPages(chapter, id) {
  ...
  try {
    ...
    for (const format of formats) {
      ...
      while (pageIndex <= 2) {
        const link = `https://img.lermanga.org/${tag}/${id}/capitulo-${chapter}/${page}${pageIndex.toString()}.${format}`;
        try {
    ...
    if (foundPage && selectedFormat) {
      ...
      while (true) {
        const link = `https://img.lermanga.org/${tag}/${id}/capitulo-${chapter}/${page}${pageIndex.toString()}.${selectedFormat}`;
      ...
      page = 0; // Set `page` to 0 if no page is found
    }
