...
export default function Rate({params}){
    //API
    ...
    const handle = (id) => {
      ...
      useEffect(() => {
        async function requestData(){
            setLoading(true)
            try {
                const res = await requestRate(page)
                setData(res.mangas)
                setLoading(false)
            } catch (error) {
                console.log(error)
...
return(
    ...
    <Column  onScroll={handleScroll} ref={scroll}  style={{overflowY: 'auto', overflowX: 'hidden', position: 'relative', backgroundColor: "#A93531" }}> 
        ...
        <Title style={{fontSize: 72, marginTop: 64, marginLeft: 44, marginBottom: 40,  fontFamily: 'Black',}}>Melhores notas</Title>
        </Row>
