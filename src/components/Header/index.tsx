...
import { Home, Library, Search, User, UserCircle } from "lucide-react";
    ...
    const Item = ({ item }) => {
        return (
            <Row style={{ alignItems: "center" }} className="bar">
                <Link href={`/${item.page}`}>
                    <BTIcon>{item.icon}</BTIcon>
                </Link>
                {openTab && (
                    <Link href={`/${item.page}`} style={{ textDecoration: "none" }}>
                        <Title style={{ fontSize: 22, color: "#F6F6F690", fontFamily: "Book" }}>
                            {item.name}
                        </Title>
                    </Link>
                )}
        ...
        >
            <Column style={{ backgroundColor: "#171717", borderRadius: 8 }}>
                <Row style={{ alignItems: "center" }}>
                    ...
                    {openTab && (
                        <Title
                            style={{ fontSize: 22, color: "#F6F6F690", fontFamily: "Book" }}
                        >
                            Mangás
                        </Title>
                    )}
