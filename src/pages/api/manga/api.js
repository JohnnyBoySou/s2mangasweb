...
export async function requestWeekend() {
   const return_data = {mangas: []}
    try {
      const response = await axios.get('https://lermanga.org/mangas/?orderby=views&order=desc',);
      console.log(response.data)
      const mangaData = clearWeekend(response.data);
      return_data.mangas = mangaData
